...
export const createPaymentOrder = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const returnUrl = `${env.FRONTEND_URL}/payment/return?order_id={order_id}`;
...
export const handlePaymentWebhook = async (req: Request, res: Response): Promise<void> => {
  ...
  const mappedStatus = CashfreeService.mapPaymentStatus(
    cfPaymentStatus || eventType?.replace('_WEBHOOK', '') || ''
  );
...
async function transitionJobToPaid(
  ...
  try {
    ...
    const updatedJob = await PrintJob.findOneAndUpdate(
      { _id: printJobId, status: 'PAYMENT_PENDING' },
      { $set: { status: 'QUEUED' } },
      { new: true }
    );
    ...
    try {
      ...
      const populatedJob = await PrintJob.findById(updatedJob._id)
    ...
    await NotificationService.create(
      ...
      'PAYMENT_SUCCESS',
      'Payment Successful',
      `Payment confirmed for ${updatedJob.publicToken}. Your order is in the vendor's queue.`,
      { jobId: updatedJob._id, token: updatedJob.publicToken }
    );
      ...
      try {
        ...
        if (vendor) {
          await NotificationService.create(
            ...
            'JOB_QUEUED',
            'New Print Job',
            `New paid print job ${updatedJob.publicToken} is waiting in your queue.`,
            { jobId: updatedJob._id, token: updatedJob.publicToken }
          );
        }
      } catch (notifErr) {
...
export const simulatePaymentSuccess = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  let payment = await Payment.findOne({ printJobId: job._id });
  if (!payment) {
