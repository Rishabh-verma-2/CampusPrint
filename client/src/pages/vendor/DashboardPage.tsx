...
const VendorDashboard: React.FC = () => {
  ...
  // Notification sound ref (reused across events)
  const audioCtxRef = useRef<AudioContext | null>(null);

  const playNewOrderSound = useCallback(() => {
    try {
      // Only play if vendor has granted audio permission (gracefully degrades)
      ...
      // Audio not available — silent failure
    }
  }, []);
  ...
  React.useEffect(() => {
    ...
    // New confirmed order from payment
    const handleNewOrder = (data: { job?: unknown }) => {
      refresh();
      toast.success('🔔 New Print Order received!', {
        duration: 6000,
        description: 'A new paid order has arrived in your queue.',
      });
      playNewOrderSound();
    };

    // On reconnect: immediately re-fetch to catch any orders missed while offline.
    // Socket.IO fires 'connect' on every successful connection (initial + reconnect).
    const handleConnect = () => {
      console.log('[VendorDashboard] Socket connected/reconnected — syncing orders');
      refresh();
    };

    socket.on('connect', handleConnect);
    socket.on('printJob:new', handleNewOrder);
    socket.on('printJob:updated', refresh);

    return () => {
      socket.off('connect', handleConnect);
      socket.off('printJob:new', handleNewOrder);
      socket.off('printJob:updated', refresh);
    };
  }, [socket, qc, playNewOrderSound]);
  ...
  // De-duplicate jobs by _id (prevent same order appearing twice from socket + polling)
  const uniqueJobs = jobs.filter(
    (job, idx, arr) => arr.findIndex((j) => j._id === job._id) === idx
  );

  // Filter unique jobs by search term
