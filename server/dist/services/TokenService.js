...
class TokenService {
     ...
     */
    static async generateDailyToken(vendorId) {
        const startOfDay = new Date();
        startOfDay.setHours(0, 0, 0, 0);
        const maxRetries = 10;
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            // Count all jobs for this vendor today (including PAYMENT_PENDING — they hold a slot)
            const countToday = await PrintJob_1.PrintJob.countDocuments({
                vendorId,
            ...
            const existing = await PrintJob_1.PrintJob.findOne({ publicToken: token }).lean();
            if (!existing)
                return token;
        }
     ...
     */
    static normalizeInputToken(input) {
